...
function Game(id,params){
    ...
    this.start = function() {
        ...
        var fn = function(){
            ...
            _context.fillStyle = '#083748';
            _context.fillRect(0,0,_.width,_.height);
            
            // Dibujar píxeles animados en el fondo (como en la página de inicio)
            if (stage && stage.status == 1) { // Solo cuando el juego está activo
                // Función para simular la animación pixelFade del CSS
                function drawAnimatedPixel(x, y, size, color, frame, delay) {
                    var animationTime = (frame + delay) % 180; // 3 segundos a 60fps
                    var progress = animationTime / 180;
                    
                    // Simular la animación ease-in-out
                    var easeProgress;
                    if (progress < 0.5) {
                    ...
                    // Calcular opacidad y escala (como en pixelFade)
                    var opacity = Math.sin(easeProgress * Math.PI);
                    var scale = easeProgress;
                    
                    if (opacity > 0.1) { // Solo dibujar si es visible
                        _context.save();
                        ...
                        var offset = (size - scaledSize) / 2;
                        
                        _context.fillRect(x + offset, y + offset, scaledSize, scaledSize);
                        _context.restore();
                    }
                }
                
                // Píxeles principales en color #1A7E7F - distribución mejorada
                for (var i = 0; i < 200; i++) {
                    // Usar distribución más uniforme para cubrir toda la pantalla
                    var x = (i * 37 + Math.floor(i / 10) * 13) % _.width;
                    var y = (i * 23 + Math.floor(i / 15) * 17) % _.height;
                    var delay = (i % 5) * 36; // Delays como en CSS
                    
                    if (i % 2 === 0) {
                        // Píxeles más brillantes
                        drawAnimatedPixel(x, y, 12, 'rgba(26, 126, 127, 0.4)', f, delay);
                    } else if (i % 3 === 0) {
                        // Píxeles suaves
                        drawAnimatedPixel(x, y, 10, 'rgba(26, 126, 127, 0.25)', f, delay);
                    } else if (i % 5 === 0) {
                        // Píxeles normales
                        drawAnimatedPixel(x, y, 14, 'rgba(26, 126, 127, 0.35)', f, delay);
                    } else {
                        // Píxeles base
                        drawAnimatedPixel(x, y, 8, 'rgba(26, 126, 127, 0.3)', f, delay);
                    }
                }
                
                // Píxeles adicionales más grandes - distribución mejorada
                for (var i = 0; i < 80; i++) {
                    // Distribución más densa para llenar espacios vacíos
                    var x = (i * 67 + Math.floor(i / 8) * 29) % _.width;
                    var y = (i * 41 + Math.floor(i / 12) * 31) % _.height;
                    var delay = (i % 4) * 45;
                    
                    // Variaciones del color #1A7E7F con diferentes opacidades
                    var opacity = 0.2 + (i % 3) * 0.1; // 0.2, 0.3, 0.4
                    drawAnimatedPixel(x, y, 16, 'rgba(26, 126, 127, ' + opacity + ')', f, delay);
                }
                
                // Píxeles extra grandes - distribución mejorada
                for (var i = 0; i < 40; i++) {
                    // Distribución más amplia para cubrir toda la pantalla
                    var x = (i * 89 + Math.floor(i / 5) * 47) % _.width;
                    var y = (i * 53 + Math.floor(i / 7) * 43) % _.height;
                    var delay = (i % 3) * 60;
                    
                    // Píxeles extra grandes con alta opacidad
                    var opacity = 0.4 + (i % 2) * 0.1; // 0.4, 0.5
                    drawAnimatedPixel(x, y, 20, 'rgba(26, 126, 127, ' + opacity + ')', f, delay);
                }
                
                // Píxeles adicionales para llenar espacios vacíos
                for (var i = 0; i < 60; i++) {
                    // Distribución aleatoria para llenar cualquier espacio vacío
                    var x = (i * 97 + f * 0.1) % _.width;
                    var y = (i * 59 + f * 0.15) % _.height;
                    var delay = (i % 6) * 30;
                    
                    var opacity = 0.15 + (i % 4) * 0.05; // 0.15, 0.2, 0.25, 0.3
                    drawAnimatedPixel(x, y, 6, 'rgba(26, 126, 127, ' + opacity + ')', f, delay);
                }
            }
            ...
            if(stage.update()!=false){		            //update返回false,则不绘制
                stage.maps.forEach(function(map){
                    ...
                    // Deshabilitar cache si se ejecuta desde file:// para evitar canvas tainted
                    var isFileProtocol = window.location.protocol === 'file:';
                    
                    if(map.cache && !isFileProtocol){
                        if(!map.imageData){
                            _context.save();
                            map.draw(_context);
